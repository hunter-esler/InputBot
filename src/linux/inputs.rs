...
impl From<KeybdKey> for u64 {
    ...
    fn from(key: KeybdKey) -> u64 {
        match key {
            ...
            NumpadAddKey => 0xffab,
            NumpadDecimalKey => 0xffae,
            NumpadDivideKey => 0xffaf,
            NumpadMultiplyKey => 0xffaa,
            NumpadSubtractKey => 0xffad,
            NumpadEnterKey => 0xff8d,
...
pub fn scan_code_to_key(scan_code: u32) -> Option<KeybdKey> {
    match scan_code {
        ...
        0x60 => Some(NumpadEnterKey),
        0x4A => Some(NumpadSubtractKey),
        0x4E => Some(NumpadAddKey),
        0x62 => Some(NumpadDivideKey),
        0x37 => Some(NumpadMultiplyKey),
        0x53 => Some(NumpadDecimalKey),
        0x3B => Some(F1Key),
        0x3C => Some(F2Key),
        0x3D => Some(F3Key),
        0x3E => Some(F4Key),
        ...
        0x7D => Some(LSuper),
        0x7E => Some(RSuper),
        _ => Some(OtherKey(scan_code as u64)),
    }
}
...
pub fn key_to_scan_code(key: KeybdKey) -> i32 {
    match key {
        ...
        NumpadDecimalKey => 0x53,
        NumpadDivideKey => 0x62,
        NumpadMultiplyKey => 0x37,
        NumpadSubtractKey => 0x4A,
        NumpadAddKey => 0x4E,
        NumpadEnterKey => 0x60,
